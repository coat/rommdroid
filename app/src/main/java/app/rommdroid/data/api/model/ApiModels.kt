...
data class CreateTokenRequest(
    val name: String,
    /** The API needs at least one scope, and answers 403 for any the user does
     *  not hold. These are RomM's READ_SCOPES, which "viewer" already has. */
...
/**
 * A user-made collection.
 *
 * `rom_ids` is deliberately not modelled: it arrives on every collection in the
 * listing, runs to thousands of ints for a big one, and the membership the app
 * stores comes from the ROM fetch instead.
 *
 * Virtual and smart collections come from sibling endpoints and are not
 * supported; RomM generates thousands of the virtual ones from metadata.
 */
@Serializable
data class CollectionSchema(
    ...
    /** Where a collection's artwork actually comes from: the singular fields
     *  above are filled only when someone uploaded a cover of their own. */
...
data class RomMetadataSchema(
    ...
    /** Aggregate score, 0-100: the mean of whichever providers scored the game,
     *  normalised by RomM (MobyGames and ScreenScraper x10, LaunchBox x20). */
